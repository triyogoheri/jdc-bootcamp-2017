...
public class CustomAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) {
        ...
        if (username.isEmpty() && userToken.getCredentials().toString().trim().isEmpty()) {
            throw new AuthenticationServiceException(messages.getMessage("JDCSecurity.usernamePasswordEmpty", "username and password is empty"));
        }

        /*check username is empty*/
        if (username.isEmpty()) {
            throw new AuthenticationServiceException(messages.getMessage("JDCSecurity.usernameEmpty", "username is empty"));
        }

        /*check password is empty*/
        if (userToken.getCredentials().toString().trim().isEmpty()) {
            throw new AuthenticationServiceException(messages.getMessage("JDCSecurity.passwordEmpty", "password is empty"));
        }
...
//                throw new AuthenticationServiceException(messages.getMessage("JDCSecurity.userEmpty", "user is not found"));
            }
        }
...
//        }
        ...
        if (isNotConflictUsername(username)) {
            throw new AuthenticationServiceException(messages.getMessage("JDCSecurity.sameUser", "this username is currently being used by another user"));
        }
...
//            throw new AuthenticationServiceException(messages.getMessage("JDCSecurity.sameWorkstation", new String[]{user.getUserDTO().getUserSpecificationDTO().getUserLoginInfoDTO().getRemoteAddress()}, "this workstation is currently being used by another user"));
//        }
        ...
        if (ldapDomain.equalsIgnoreCase(ldapDefault)) {
            ...
            if (!user.isEnabled()) {
                throw new DisabledException(messages.getMessage("JDCSecurity.disabled", "User is disabled"));
            }
            ...
            if (!new BCryptPasswordEncoder().matches(authentication.getCredentials().toString(), user.getPassword())) {
                ...
                throw new BadCredentialsException(messages.getMessage("JDCSecurity.badCredentials", "Bad credentials"));
            }

            /*check password expired*/
            if (!user.isCredentialsNonExpired()) {
                throw new CredentialsExpiredException(messages.getMessage("JDCSecurity.credentialsExpired", "User account has expired"));
            }

            /*check account locked*/
            if (!user.isAccountNonLocked()) {
                throw new LockedException(messages.getMessage("JDCSecurity.locked", "User account is locked"));
            }
        }

        /*check role status*/
        if (!user.getUserDTO().getRoleDTO().getRoleStatus().equals(StatusData.ACTIVE)) {
            throw new AuthenticationServiceException(messages.getMessage("JDCSecurity.role.disable", "avantrade : user group is disabled"));
        }

        /*check is this day accessible*/
        if (!isThisDayAccessibleForThisUser(user.getUserDTO().getUserSpecificationDTO().getAccessTimeDTO())) {
            throw new AuthenticationServiceException(messages.getMessage("JDCSecurity.noAccess", "No Access"));
        }
...
//            throw new AuthenticationServiceException(messages.getMessage(e.getMessage()));
    ...
    public boolean isNotConflictIP(String ip) {
        try {
            ...
            /* if connection is refused */
            throw new AuthenticationServiceException(messages.getMessage("JDCSecurity", "avantrade : Connection is refused"));
        }
    }
